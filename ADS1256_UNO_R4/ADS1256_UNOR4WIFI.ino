  ...
  */
  ...
  //     case 'C':                       //Cycle single ended inputs (A0+GND, A1+GND ... A7+GND)
       ...
       // {
          ...
          for (int j = 0; j < 8; j++) {
            channels[j] = A.cycleSingle()/1000000000.0;//A.convertToVoltage(A.cycleSingle());  //store the converted single-ended results in the buffer
          }
          for (int i = 0; i < 8; i++) {
            Output=Output+String((channels[i]));  //print the converted single-ended results with 4 digits

            if (i < 7)  //Only printing tab between the first 7 conversions
            {
              Output=Output+",";  //tab separator to separate the 8 conversions shown in the same line
            }
          }
          udp.beginPacket(hostIp, hostPort);
          udp.print(Output);
          udp.endPacket();
;  //Printing a linebreak - this will put the next 8 conversions in a new line
      //  }
      A.stopConversion();
      delay(100);
    //     break;
